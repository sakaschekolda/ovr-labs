    ...
    if (!hasUpdate) {
      if (Object.keys(req.body).length > 0) {
        ...
        if (invalidFields.length > 0) {
          throw new ValidationError(
            {
              fields: `Invalid or non-updatable fields provided: ${invalidFields.join(', ')}. Allowed fields are: ${allowedFields.join(', ')}.`,
            },
            'Invalid update request.',
          );
        } else {
          throw new ValidationError(
            {
              general:
                'No valid fields provided for update. Allowed fields are: title, description, date, category.',
            },
            'Invalid update request.',
          );
        }
