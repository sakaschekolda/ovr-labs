...
interface MiddlewareError extends Error {
    status?: number;
    expose?: boolean;
    errors?: ValidationErrorsObject | Array<{ path?: string | null; message: string }>;
}
...
const errorHandler = (err: MiddlewareError | ApiError | Error, req: Request & { user?: RequestUser }, res: Response, _next: NextFunction): void => {
  console.error('--------------------');
  ...
  if ('errors' in err && err.errors) {
      try {
            console.error(`Validation/Sequelize Errors: ${JSON.stringify(err.errors, null, 2)}`);
      } catch (stringifyError: unknown) {
            console.error(`Error trying to stringify errors property: ${stringifyError instanceof Error ? stringifyError.message : String(stringifyError)}`);
  ...
  if (err instanceof ValidationError) {
    res.status(400).json({
      ...
      errors: (err.errors && typeof err.errors === 'object' && !Array.isArray(err.errors)) ? err.errors : undefined
    });
    return;
  }
  ...
  if (err.name === 'SequelizeValidationError' || err.name === 'SequelizeUniqueConstraintError') {
    ...
    if ('errors' in err && Array.isArray(err.errors)) {
      (err.errors as Array<{ path?: string | null; message: string }>).forEach((e) => {
        if (e.path && typeof e.path === 'string') {
          errors[e.path] = e.message;
        } else {
          const generalKey = 'general';
          const existingMessage = typeof errors[generalKey] === 'string' ? errors[generalKey] : '';
  ...
  if (err instanceof ApiError) {
    res.status(err.statusCode || 500).json({
      ...
      ...(err.errors && typeof err.errors === 'object' && !Array.isArray(err.errors) && { errors: err.errors })
    });
    return;
  }

  console.log('[errorHandler] No specific match found, defaulting to 500.');
  const responseMessage = 'Internal Server Error. An unexpected issue occurred.';
